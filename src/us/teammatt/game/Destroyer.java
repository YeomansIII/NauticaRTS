...
public class Destroyer extends Unit {
	...
	Unit shootAt;
		...
		public Destroyer(BlockMap map, ViewPort view,ArrayList<Unit> selectedUnits,ArrayList<Unit> allUnits) throws SlickException {
			this.selectedUnits = selectedUnits;
			this.allUnits = allUnits;
			bulletList = new ArrayList<Integer>();
			image = new Image("data/destroyer.png");
		...
		scoutPoly = new Polygon(new float[] { playerX, playerY+30,
				playerX+30, playerY+30, playerX + 30, playerY + 60,
				playerX, playerY + 60 });
	...
	public void move(int endX, int endY) {
		...
		/*
		 * int startX = playerX/8; int startY = playerY/8; endX = endX/8;
		 * endY = endY/8; path = pathFinder.findPath(null, startX, startY,
		 * endX, endY); if(path!=null) {
		 ...
		 * ".");
		 * 
		 * playerX = path.getX(i)*8; playerY = path.getY(i)*8;
		 * scoutPoly.setX(playerX); scoutPoly.setY(playerY); } }
		 */
	}
