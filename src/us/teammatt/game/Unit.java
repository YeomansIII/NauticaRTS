...
public class Unit{
	...
	static int hp, damage, reloadTime, cost, speed;
	int offset;
	Image image,bullet;
	Path path = null;
	...
	public Unit(BlockMap map, ViewPort view,ArrayList<Unit> selectedUnits,ArrayList<Unit> allUnits,int[] stats,String iimage) throws SlickException {
		this.selectedUnits = selectedUnits;
		this.allUnits = allUnits;
		bulletList = new ArrayList<Integer>();
		image = new Image(iimage);
		hp=stats[0]; damage=stats[1]; reloadTime=stats[2]; cost=stats[3]; speed=stats[4]; offset=stats[5];
		System.out.println(offset);
		bullet = new Image("data/bullet.png");
		image.setRotation(90);
		viewport = view;
		pTileX=25; pTileY=120;
		...
		scoutPoly = new Polygon(new float[] { playerX, playerY + offset,
				playerX + 25, playerY + offset, playerX + 25, playerY + offset+25,
				playerX, playerY + offset+25 });
		...
		thread = new Thread() {
			...
			public synchronized void run() { 
				...
				playerY = (int) ((scoutPoly.getY())-offset);
					if(move) {
